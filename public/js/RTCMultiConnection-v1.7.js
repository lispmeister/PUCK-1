// Last time updated at April 29, 2014, 08:20:23
...
-. RTCMultiConnection MUST always display custom media-prompt dialog box on HTTPs
.  It MUST NEVER auto get media devices; Google's HTTPs sometimes behaves weird.
.  RTCMultiConnection MUST focus over user instead of programmers :)
.  Users' privacy MUST be a highest-priority feature in RTCMultiConnection!

-. using DetectRTC to check support of microphone and webcam
.   if there is no microphone; then it auto skips it!
.    -. if system doesn't support audio; auto join with only video
.    -. if system doesn't support video; auto join with only audio

-. auto injecting VoiceActivityDetection:true for chrome to make sure c=IN is always present; otherwise b=AS will fail.
...
-. connection.DetectRTC.hasMicrophone || connection.DetectRTC.hasWebcam
-. connection.trickleIce = false; // for SIP/XMPP/XHR signaling
...
(function() {
    ...
    function PeerConnection() {
        return {
            ...
            setConstraints: function() {
                ...
                // temporarily disabling ipv6 support
                if (false && isChrome && chromeVersion >= 32 && !isNodeWebkit) {
    ...
    function setDefaults(connection) {
        ...
        if (isFirefox) {
            ...
            iceServers.push({ url: 'stun:stun.services.mozilla.com' });
        }
        ...
        if (isChrome && chromeVersion < 28) {
            iceServers.push({ url: 'turn:homeo@turn.bistri.com:80?transport=udp', credential: 'homeo' });
            iceServers.push({ url: 'turn:homeo@turn.bistri.com:80?transport=tcp', credential: 'homeo' });
        }

        if (isChrome && chromeVersion >= 28) {
            iceServers.push({ url: 'turn:turn.bistri.com:80?transport=udp', credential: 'homeo', username: 'homeo' });

            iceServers.push({ url: 'turn:turn.bistri.com:80?transport=tcp', credential: 'homeo', username: 'homeo' });

            iceServers.push({ url: 'turn:turn.anyfirewall.com:443?transport=tcp', credential: 'webrtc', username: 'webrtc' });
        }

        // added... no external for now...
        iceServers = []
        ...
        connection.getDevices = function(callback) {
            ...
            // 1st step: verify "MediaStreamTrack" support.
            if (!window.MediaStreamTrack) return callback(connection.devices);
