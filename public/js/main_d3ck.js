...
$(document).ready(function () {
    ...
    do_that_rtc_thang(window.location.hostname)
...
peer    = {}
userid  = psuedo_r_string()

function do_that_rtc_thang(host) {

    // selector switches 4 fun
    $('#video_effect').change(function() {
        var val = $('#video_effect option:selected').val();
        console.log('selected: ' + val)

        if (val == "clear") {
            $('video').css({'-webkit-filter': ""})
        }
        else {
    ...
    // from here plus some bugs I surely introduced:
    ...
    websocket.onmessage = function (evt) { console.log("wss received "+evt.data) }
    websocket.onerror = function (e)     { console.log("wss Error") ;console.log(e) }


    // var websocket = new WebSocket('wss://www.webrtc-experiment.com:8563');
    websocket.onopen = function() {
        console.log("wss opened, sending channel to server")
    ...
    peer.onUserFound = function(userid) {
        ...
        button.onclick = function() {
            ...
            // show the effects only after video is up
            $('#video_effect_div').removeClass()
